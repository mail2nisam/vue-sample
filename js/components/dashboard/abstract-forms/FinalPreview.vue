        ...
        <div class="container container-small block-second">
            <div class="box-white">
                <header class="content-header box-item">
                    ...
                    <div class="right-side control-box" v-if="$route.name==='preview'">
                        <a href="#" class="edit-btn btn-with-bg" @click.prevent="editPeople"><i class="icon-edit"></i>Edit</a>
                    </div>
                </header>
                <div class="box-item">
                    <div class="card-holder-wrap">
                        <template v-for="(person,index) in people">
                            <user-card :people="person" :key="index" :index="index"/>
                        </template>
                    </div>
                </div>
            </div>
            <footer class="content-footer" v-if="$route.name==='preview'">
...
<script>
    ...
    export default {
        name: "FinalPreview",
        components: {AbstractActionBox, UserCard, AbstractPreview},
        computed: {
            /**
        ...
        methods: {
            editPeople: function () {
                this.$router.push({name: 'author', params: {id: this.$route.params.id}});
            },
            submit: function () {
                if (this.step < 3) {
                    this.$notify({
